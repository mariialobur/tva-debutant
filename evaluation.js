...
function submitExam(e){
  ...
  const passed=score>=PASS_SCORE,result={score,total:EXAM_SIZE,percent:Math.round(score/EXAM_SIZE*100),passed,date:new Date().toISOString(),review,assessment:'blueprint-v1'};
...
function renderAttestation(name){
  ...
  wrap.innerHTML=`<div class="attest-actions"><button class="btn" id="closeAttest">Fermer</button><button class="btn primary" id="printAttest">Imprimer / enregistrer en PDF</button></div><section class="attest-page page-one"><div class="attest-kicker">Parcours pédagogique indépendant</div><h1>ATTESTATION DE PARCOURS</h1><h2>Méthode effective de la TVA suisse · Niveau 1</h2><p class="attest-name">${esc(name)}</p><p class="attest-main">a validé les <strong>${totalCases} cas pratiques en mode Évaluation</strong> et a réussi l’auto-évaluation finale structurée du Niveau 1 consacré au remplissage de déclarations TVA selon la méthode effective.</p><div class="attest-stats"><div><strong>${totalCases} / ${totalCases}</strong><span>cas validés à 100 %</span></div><div><strong>${score} / ${EXAM_SIZE}</strong><span>évaluation finale</span></div><div><strong>${date}</strong><span>date du résultat</span></div></div><div class="attest-themes">${themes.map(t=>`<span>${esc(t[0])}</span>`).join('')}</div><p class="attest-url">${PROJECT_URL}</p><p class="attest-disclaimer">Cette attestation confirme uniquement l’achèvement de ce parcours d’entraînement indépendant à la déclaration TVA et la réussite de son auto-évaluation finale. Elle ne constitue ni un diplôme, ni un titre professionnel, ni une certification reconnue ou accréditée. Ce projet est indépendant et sans affiliation avec l’AFC/ESTV ou le SEFRI. Le nom est saisi par le participant; son identité n’est pas vérifiée. Le résultat est généré localement et n’est pas vérifiable par un tiers auprès du site.</p></section><section class="attest-page page-two"><h1>RELEVÉ DU PARCOURS</h1><p class="attest-sub">Contenu et thèmes travaillés dans l’entraînement au décompte TVA.</p><div class="attest-person"><strong>${esc(name)}</strong><span>${totalCases} cas validés · résultat final ${score}/${EXAM_SIZE}</span></div><div class="attest-grid">${themes.map(t=>`<article><h3>${esc(t[0])}</h3><p>${esc(t[1])}</p></article>`).join('')}</div><div class="attest-reference"><strong>Référentiel pédagogique</strong><p>LTVA · OTVA · prototype et publications AFC utilisés dans les cas. Les sources officielles et la pratique en vigueur restent déterminantes pour un dossier réel.</p><p>${PROJECT_URL}</p></div><p class="attest-disclaimer">Ce relevé décrit les thèmes abordés par le trainer de déclaration. Il n’atteste pas de compétences professionnelles générales, d’un diplôme ou d’une certification reconnue. Il est généré localement à partir de la progression et du résultat enregistrés dans le navigateur.</p></section>`;
