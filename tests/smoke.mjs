...
for(const token of ['app.js?v=2.1.0','evaluation.js?v=3.2.0','progress-ui.js?v=1.4.0','styles.css?v=2.0.0','evaluation.css?v=1.0.0','Mémo professionnel','finalEvaluation','0 / 18 maîtrisés'])assert.ok(html.includes(token),`Missing token: ${token}`);

const evalJs=fs.readFileSync('evaluation.js','utf8');
for(const token of ['ATTESTATION DE RÉUSSITE DU PARCOURS','tva_effective_final_evaluation_v3_blueprint','meilleur résultat conservé','Niveau 1','selectBlueprintQuestions','5 blocs de compétences','ce repère n’est pas une condition d’accès','Les réponses attendues ne sont pas affichées après un échec','remplissage de déclarations TVA','level1-extension.js'])assert.ok(evalJs.includes(token),`Missing evaluation token: ${token}`);
